...
def TransformToDF(CardDict):
    ...
    # If we want to, in order to get the SetID, we can just go ahead and grab the data of
...
def StartScrape(promoURL, setName, startingPage):
    ...
    try:
        ...
        while scraper.loop_var:
            ...
            page_url = f"{base_URL}?view=grid&productLineName=one-piece-card-game&setName={setName}&ProductTypeName=Cards&page={page_num}&CardType=Character&Color=Yellow"

	    #productLineName=one-piece-card-game&productTypeName=Cards&page=1&view=grid&setName=one-piece-promotion-cards&CardType=Event
